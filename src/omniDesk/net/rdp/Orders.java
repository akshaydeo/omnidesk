...
import omniDesk.rdp.orders.DeskSaveOrder;
import omniDesk.rdp.orders.DestBltOrder;
import omniDesk.rdp.orders.LineOrder;
import omniDesk.rdp.orders.MemBltOrder;
import omniDesk.rdp.orders.PatBltOrder;
import omniDesk.rdp.orders.Pen;
import omniDesk.rdp.orders.PolyLineOrder;
import omniDesk.rdp.orders.RectangleOrder;
import omniDesk.rdp.orders.ScreenBltOrder;
import omniDesk.rdp.orders.Text2Order;
import omniDesk.rdp.orders.TriBltOrder;
...
public class Orders {
	...
	public void processOrders(RdpPacket_Localised data, int next_packet,
		...
		int present = 0;
		// int n_orders = 0;
		int control_flags = 0, order_type = 0;
		int size = 0, processed = 0;
		boolean delta;

		while (processed < n_orders) {

			control_flags = data.get8();

			if ((control_flags & RDP_ORDER_STANDARD) == 0) {
				System.out.println("control flag=" + control_flags);
				throw new OrderException("Order parsing failed!");
				// continue;
			}

			if ((control_flags & RDP_ORDER_SECONDARY) != 0) {
				this.processSecondaryOrders(data);
			} else {
				Log.d("order.java", "primary order");
				if ((control_flags & RDP_ORDER_CHANGE) != 0) {
				...
				present = this.inPresent(data, control_flags, size);

				if ((control_flags & RDP_ORDER_BOUNDS) != 0) {

					if ((control_flags & RDP_ORDER_LASTBOUNDS) == 0) {
						this.parseBounds(data, os.getBounds());
					}

					Common.currentImageViewer.setClip(os.getBounds());
				}

				delta = ((control_flags & RDP_ORDER_DELTA) != 0);
				...
				case RDP_ORDER_PATBLT:
					...
					// Toast.makeText(Common.acitvity_context,"pat blt",
					// 1).show();

					Log.d("order.java", "pat blt");
				...
				case RDP_ORDER_LINE:
					// pras logger.debug("Line Order");
					Log.d("order.java", "LINE ORDER RECEIVED");
					this.processLine(data, os.getLine(), present, delta);
					break;

				case RDP_ORDER_RECT:
					// pras logger.debug("Rectangle Order");
					// Toast.makeText(Common.acitvity_context,"rectangle",
					// 1).show();
					Log.d("order.java", "rect");
					this.processRectangle(data, os.getRectangle(), present,
							delta);
					break;

				case RDP_ORDER_DESKSAVE:
					// pras logger.debug("Desksave!");
					Log.d("order.java", "desk save");
				...
				case RDP_ORDER_TEXT2:
					// pras logger.debug("Text2 Order");
					System.out.println("TEXT2 ORDER");
					this.processText2(data, os.getText2(), present, delta);
					break;

				default:
					// pras logger.warn("Unimplemented Order type " +
					// order_type);
					return;
				}

				if ((control_flags & RDP_ORDER_BOUNDS) != 0) {
					// pras surface.resetClip();
					// pras logger.debug("Reset clip");
				}
			}
	...
	private void processSecondaryOrders(RdpPacket_Localised data)
		...
		System.out.println("Inside secondary orders processing");
		int length = 0;
		int type = 0;
		int flags = 0;
		int next_order = 0;
		...
		case RDP_ORDER_RAW_BMPCACHE:
			System.out.println("Raw BitmapCache Order");
			this.processRawBitmapCache(data);
			break;

		case RDP_ORDER_COLCACHE:
			System.out.println("Colorcache Order");
			this.processColorCache(data);
			break;

		case RDP_ORDER_BMPCACHE:
			System.out.println("Bitmapcache Order");
			this.processBitmapCache(data);
			break;

		case RDP_ORDER_FONTCACHE:
			System.out.println("Fontcache Order");
		...
		default:
			Log.w("orders.java", "Unimplemented 2ry Order type " + type);
		}
	...
	private void processColorCache(RdpPacket_Localised data)
		...
		// blue);
		ColorModel256 cm=new ColorModel256();
	...
	private void processBitmapCache(RdpPacket_Localised data)
		...
		try {
			if (Bpp == 1) {
				byte[] pixel = Bitmap
						.decompress(width, height, size, data, Bpp);
				if (pixel != null)
					cache.putBitmap(cache_id, cache_idx, new Bitmap(Bitmap
							.convertImage(pixel, Bpp), width, height, 0, 0), 0);
				else
					Log.w("orders.java", "Failed to decompress bitmap");
			} else {
				int[] pixel = Bitmap.decompressInt(width, height, size, data,
						Bpp);
				if (pixel != null)
					cache.putBitmap(cache_id, cache_idx, new Bitmap(pixel,
							width, height, 0, 0), 0);
				else
					Log.w("orders.java", "Failed to decompress bitmap");
			}
		} catch (Exception e) {
			System.out.println("Exception in orders.java/processBitmapCache"
					+ e);
		}
	}
	...
	private void process_bmpcache2(RdpPacket_Localised data, int flags,
		...
		Log.i("orders.java", "BMPCACHE2(compr=" + compressed + ",flags="
				+ flags + ",cx=" + width + ",cy=" + height + ",id=" + cache_id
		...
		{
			System.out.println("Exception caught in order.java/processBMPcache2" + e);
		}
			if (bmpdataInt == null) {
				System.out.println("Failed to decompress bitmap data");
				// xfree(bmpdata);
				return;
			}
		...
		if (bitmap != null) {
			...
			System.out.println("process_bmpcache2: ui_create_bitmap failed");
		}
	...
	private void processText2(RdpPacket_Localised data, Text2Order text2,
		...
		if ((present & 0x010000) != 0) {
			...
			System.out
					.println("Unknown order state member (0x010000) in text2 order.\n");
		}
	...
	private void drawText(Text2Order text2, int clipcx, int clipcy, int boxcx,
		...
		if (boxcx > 1) {
			Common.currentImageViewer.fillRectangle(text2.getBoxLeft(), text2
					.getBoxTop(), boxcx, boxcy, text2.getBackgroundColor());
		} else if (text2.getMixmode() == MIX_OPAQUE) {
			Common.currentImageViewer.fillRectangle(text2.getClipLeft(), text2
					.getClipTop(), clipcx, clipcy, text2.getBackgroundColor());
		}

		/*
		 * System.out.println("X: " + text2.getX() + " Y: " + text2.getY() +
		...
		for (int i = 0; i < length;) {
			...
			case (0xfe):
				...
				for (int j = 0; j < entry.getSize(); j++) {
					...
					if (glyph != null) {
						...
						Common.currentImageViewer.drawGlyph(text2.getMixmode(),
								x + (short) glyph.getOffset(), y
										+ (short) glyph.getBaseLine(), glyph
										.getWidth(), glyph.getHeight(), glyph
										.getFontData(), text2
			...
			default:
				...
				if (glyph != null) {
					Common.currentImageViewer.drawGlyph(text2.getMixmode(), x
							+ (short) glyph.getOffset(), y
	...
	private void processRectangle(RdpPacket_Localised data,
		...
		Common.currentImageViewer.drawRectangleOrder(rect);
	...
	private void processLine(RdpPacket_Localised data, LineOrder line,
		...
		if (line.getOpcode() < 0x01 || line.getOpcode() > 0x10) {
			Log.w("orders.java/processLine", "bad ROP2 0x" + line.getOpcode());
			return;
		}

		// now draw the line
		Common.currentImageViewer.drawLineOrder(line);
	...
	private void processPatBlt(RdpPacket_Localised data, PatBltOrder patblt,
		...
		Common.currentImageViewer.drawPatBltOrder(patblt);
	}
