...
public class MCS {
	 ...
	 * @throws RdesktopException
	 * @throws prasad.con_req.CryptoException
	 */
	public void connect(InetAddress host, int port, RdpPacket_Localised data)
			throws IOException, OmniDeskException, OrderException,
			CryptoException, RdesktopException {
		...
		this.sendConnectInitial(data); // Client MCS Connect Initial PDU with
										// GCC Conference Create Request
		this.receiveConnectResponse(data);// Server MCS Connect Response PDU
		// with GCC Conference Create Response

		send_edrq(); // Erect Domain Request
		send_aurq();// Client MCS Attach User Request PDU

		this.McsUserID = receive_aucf();// Server MCS Attach User Confirm PDU
		send_cjrq(this.McsUserID + MCS_USERCHANNEL_BASE);// Client MCS Channel
															// Join Request PDU
		receive_cjcf();// SERVER-CLIENT->Client MCS Channel Join Confirm PDU
		send_cjrq(MCS_GLOBAL_CHANNEL);// same on global channel
		receive_cjcf();
	 ...
	 * @throws RdesktopException
	 */
	public RdpPacket_Localised init(int length) throws OmniDeskException,
			RdesktopException {
	 ...
	 * @throws RdesktopException
	 */
	public void send(RdpPacket_Localised buffer) throws OmniDeskException,
			IOException, RdesktopException {
	...
	public void send_to_channel(RdpPacket_Localised buffer, int channel)
			throws OmniDeskException, IOException, RdesktopException {
		int length = 0;
	...
	public RdpPacket_Localised receive(int[] channel) throws IOException,
			OmniDeskException, OrderException, CryptoException,
			RdesktopException {
		...
		if (appid != SDIN) {
			if (appid != DPUM) {
				throw new OmniDeskException("Expected data got" + opcode);
			}
	...
	public void sendConnectInitial(RdpPacket_Localised data)
			throws IOException, OmniDeskException, RdesktopException {
	...
	public void receiveConnectResponse(RdpPacket_Localised data)
			throws IOException, OmniDeskException, OrderException,
			CryptoException, RdesktopException, CryptoException {
		...
		if (result != 0) {
			...
			throw new OmniDeskException("MCS Connect failed: "
	 ...
	 * @throws RdesktopException
	 */
	public void send_edrq() throws IOException, OmniDeskException,
			RdesktopException {
	 ...
	 * @throws RdesktopException
	 */
	public void send_cjrq(int channelid) throws IOException, OmniDeskException,
			RdesktopException {
	 ...
	 * @throws RdesktopException
	 */
	public void send_aucf() throws IOException, OmniDeskException,
			RdesktopException {
	 ...
	 * @throws RdesktopException
	 */
	public void send_aurq() throws IOException, OmniDeskException {
		...
		try {
			IsoLayer.send(buffer);
			Log.d("MCS.java", "Client MCS Attach User Request PDU sent!!");
	 ...
	 * @throws RdesktopException
	 */
	public void receive_cjcf() throws IOException, OmniDeskException,
			OrderException, CryptoException, RdesktopException {
		int opcode = 0, result = 0;
		RdpPacket_Localised buffer = IsoLayer.receive();
		opcode = buffer.get8();
		if ((opcode >> 2) != CJCF) {
			throw new OmniDeskException("Expected CJCF got" + opcode);
		}

		result = buffer.get8();
		if (result != 0) {
			throw new OmniDeskException("Expected CJRQ got " + result);
		}
	 ...
	 * @throws RdesktopException
	 */
	public int receive_aucf() throws IOException, OmniDeskException,
			OrderException, CryptoException, RdesktopException {
		...
		opcode = buffer.get8();
		if ((opcode >> 2) != AUCF) {
			throw new OmniDeskException("Expected AUCF got " + opcode);
		}

		result = buffer.get8();
		if (result != 0) {
			throw new OmniDeskException("Expected AURQ got " + result);
		}
	...
	public int berParseHeader(RdpPacket_Localised data, int tagval)
		...
		if (tag != tagval) {
			throw new OmniDeskException("Unexpected tag got " + tag
					+ " expected " + tagval);
		}
