 ...
 * @author akshay
 * stub
 * implemetation is spacific to android
 */

/**
 * buffered image has some equivalent in the android java
 * find exact working of the bufferedImage
 */
public class Bitmap {
	 public int usage;
	     ...
	     */
		//TODO: convert this method into ANDROID specific manner
		 ...
		 */
	    public static WrappedImage decompressImgDirect(int width, int height, int size, RdpPacket_Localised data, int Bpp, int left, int top, WrappedImage w) throws OmniDeskException {
	       ...
	       // WrappedImage w = null;
	        ...
	        while (input < end) {
	            ...
	            while (count > 0) {
	               if (x >= width) {
	                   if (height <= 0) throw new OmniDeskException("Decompressing bitmap failed! Height = " + height);
	                    x = 0;
	                    height--;
	     ...
	     */
	    public static android.graphics.Bitmap decompressImg(int width, int height, int size, RdpPacket_Localised data, int Bpp) throws RdesktopException, OmniDeskException {
	    	Log.d("bitmap.java","decompressImg");
	        WrappedImage w = null;
	       ...
	       // if(cm == null)
	            w = new WrappedImage(width, height);/*,android.graphics.Bitmap.Config.RGB_565);/*, BufferedImage.TYPE_INT_RGB);*/
	        //else
	     ...
	     * @throws RdesktopException
	     */
		public static int[] decompressInt(int width, int height, int size, RdpPacket_Localised data, int Bpp) throws OmniDeskException {
			Log.d("bitmap.java","decompressInt");
			byte[] compressed_pixel = new byte[size];
			data.copyToByteArray(compressed_pixel, 0, data.getPosition(), size);
			data.incrementPosition(size);
	     ...
	     * @throws RdesktopException
	     */
		public static byte[] decompress(int width, int height, int size, RdpPacket_Localised data, int Bpp) throws OmniDeskException {
			...
			while (input < end) {
				...
				while (count > 0) {
					if (x >= width) {
						if (height <= 0)
							throw new OmniDeskException("Decompressing bitmap failed! Height = " + height);
					...
					case 0xe: /* Black */
						...
						break;
					default:
						throw new OmniDeskException("Unimplemented decompress opcode " + opcode);// ;
					}
				}
			}
